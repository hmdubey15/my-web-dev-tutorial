...
- <b style="color: Chartreuse;">grid-template</b>: This property is a shorthand property for the following properties: `grid-template: none|grid-template-rows / grid-template-columns|grid-template-areas|initial|inherit;`
...
- <b style="color: Chartreuse;">grid-column</b> & <b style="color: Chartreuse;">grid-row</b>: Shorthand for `grid-column-start` + `grid-column-end`, and `grid-row-start` + `grid-row-end`, respectively.
    - **start-line / end-line** – Each one accepts all the same values as the longhand version, including `span`. We can give value `-1` as well to indicate the end. 
For just above grid:
