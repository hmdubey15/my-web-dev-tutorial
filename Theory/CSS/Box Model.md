...
<h3 style="text-decoration: underline;">Height & Width</h3>
...
<b style="color: Chartreuse;">The height and width of an inline element cannot be set in CSS. Their dimensions are usually determined by the content they contain.</b>
...
<h3 style="text-decoration: underline;"><a src="https://www.w3schools.com/css/css_overflow.asp">Overflow</a> handling</h3>
...
<b style="color: Chartreuse;">Note: The overflow property only works for block elements with a specified height.</b>
...
- <b style="color: Salmon;">border-style</b>: Specifies the style of the border, such as solid, dashed, dotted, etc. This property can also have from one to four values like above.
  <b style="color: Chartreuse;">None of the OTHER CSS border properties (which you will learn more about in the next chapters) will have ANY effect unless the border-style property is set!</b>
...
The `outline-offset` property adds space between an outline and the edge/border of an element. The space between an element and its outline is transparent. <b style="color: Chartreuse;">It takes just 1 value, not 4!</b>
...
<b style="color: Chartreuse;">Note: Negative values are not allowed.</b>
...
<b style="color: Chartreuse;">NOTE</b>: The margin: auto; technique for centering using block-level elements won't work directly with inline elements like `<span>`. The auto value for margin is used for block-level elements that have a specified width. For inline elements, you might want to use other techniques.
