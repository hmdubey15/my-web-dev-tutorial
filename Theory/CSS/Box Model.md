...
- <b style="color: Salmon;">border-style</b>: Specifies the style of the border, such as solid, dashed, dotted, etc. This property can also have from one to four values like above.
  <span style="color: Chartreuse;">None of the OTHER CSS border properties (which you will learn more about in the next chapters) will have ANY effect unless the border-style property is set!</span
...
<b style="color: Chartreuse;">NOTE</b>: The `margin: auto;` technique for centering using block-level elements won't work directly with inline elements like `<span>`. It is used for block-level elements that have a specified width. For inline elements, you might want to use other techniques.
