...
- <b style="color: LightCoral;">DOMContentLoaded</b>: It fires when the initial HTML document has been completely parsed and loaded by the browser, and the Document Object Model (DOM) is fully constructed. However, this event is fired before all external resources such as images and stylesheets are fully loaded. It is document event.
- <b style="color: LightCoral;">load</b>: Signifies that the entire page content, including all external resources, is ready for interaction and rendering. It is window event.
- <b style="color: LightCoral;">beforeunload</b>: Occurs just before the page is unloaded or navigated away from. It allows developers to execute code or prompt the user for confirmation before they leave the current page, potentially preventing accidental navigation or loss of unsaved data. It is window event.
...
- <b style="color: Salmon;">getElementById(id):</b> Retrieves an element node by its unique ID attribute.
- <b style="color: Salmon;">getElementByClassName(className):</b> Returns collection of elements that have the given CSS class.
- <b style="color: Salmon;">getElementByTagName(tagName):</b> Returns collection of elements with the given tag. The tag parameter can also be a star `*` for “any tags”.
- <b style="color: Salmon;">document.getElementsByName(name):</b> Returns elements with the given `name` attribute, document-wide. Very rarely used.
- <b style="color: Salmon;">elem.querySelectorAll(css):</b> Returns all elements inside `elem` matching the given CSS selector.
- <b style="color: Salmon;">elem.querySelector(css):</b> Returns the first element for the given CSS selector. Unlike other methods, it gives <span style="color: Fuchsia;">static</span> collection. It’s like a fixed array of elements.
- <b style="color: Salmon;">elem.matches(css):</b> Previous methods were searching the DOM. This method does not look for anything, it merely checks if `elem` matches the given CSS-selector. It returns true or false.
- <b style="color: Salmon;">elem.closest(css):</b> Looks for the nearest ancestor that matches the CSS-selector. The elem itself is also included in the search.

<br>

# Node properties and DOM Manipulation

- <b style="color: Aquamarine;">elem.nodeName, elem.tagName:</b> `document.body.nodeName` and `document.body.tagNames` both return `BODY`. The difference between them is subtle - `tagName` property exists only for Element nodes while the `nodeName` is defined for any Node. Returned value is always uppercase.
- <b style="color: Aquamarine;">elem.innerHTML:</b> This property allows to get the HTML inside the element as a string. We can also modify it. So it’s one of the most powerful ways to change the page.
...
- <b style="color: Aquamarine;">elem.outerHTML:</b> This property contains the full HTML of the element. That’s like `innerHTML` plus the element itself. Similar to `innerHTML`, we can assign values here as well.
- <b style="color: Aquamarine;">elem.textContent:</b> Provides access to the text inside the element - only text, minus all `<tags>`. Writing to textContent is much more useful, because it allows to write text the “safe way”.
...
- <b style="color: Aquamarine;">elem.nodeType:</b> This property provides one more, “old-fashioned” way to get the “type” of a DOM node. It's value is `1` for element nodes, `3` for text nodes, `9` for document object and few others not very important.
...
- <b style="color: Orchid;">elem.hasAttribute(name)</b> – checks for existence.
- <b style="color: Orchid;">elem.getAttribute(name)</b> – gets the value.
- <b style="color: Orchid;">elem.setAttribute(name, value)</b> – sets the value as string.
- <b style="color: Orchid;">elem.removeAttribute(name)</b> – removes the attribute.
...
- <b style="color: Salmon;">node.append(<span style="color: royalblue; font-weight: normal;">nodes or strings</span>)</b> – append nodes or strings at the end of node,
- <b style="color: Salmon;">node.prepend(<span style="color: royalblue; font-weight: normal;">nodes or strings</span>)</b> - insert nodes or strings at the beginning of node,
- <b style="color: Salmon;">node.before(<span style="color: royalblue; font-weight: normal;">nodes or strings</span>)</b> - insert nodes or strings before node,
- <b style="color: Salmon;">node.after(<span style="color: royalblue; font-weight: normal;">nodes or strings</span>)</b> - insert nodes or strings after node,
- <b style="color: Salmon;">node.replaceWith(<span style="color: royalblue; font-weight: normal;">nodes or strings</span>)</b> – replaces node with the given nodes or strings.
- <b style="color: Salmon;">node.remove()</b> - Removes the node.
- <b style="color: Salmon;">elem.insertAdjacentHTML(<span style="color: royalblue; font-weight: normal;">where, html</span>)</b> - to insert an HTML string “as html”, with all tags and stuff working. The first parameter is a string code word, specifying where to insert relative to elem. Must be one of the following:
...
- <b style="color: Salmon;">elem.insertAdjacentText(<span style="color: royalblue; font-weight: normal;">where, text</span>)</b> - the same syntax, but a string of `text` is inserted “as text” instead of `HTML`.
- <b style="color: Salmon;">elem.insertAdjacentElement(<span style="color: royalblue; font-weight: normal;">where, elem</span>)</b> - the same syntax, but inserts an `element`.
- <b style="color: Salmon;">elem.cloneNode(<span style="color: royalblue; font-weight: normal;">true</span>)</b> - creates a “deep” clone of the element – with all attributes and subelements. If we call `elem.cloneNode(false)`, then the clone is made without child elements.
...
- <b style="color: Plum;">elem.className</b>: Corresponds to the "class" attribute. Assigning value to it will replace entire string of classes i.e. if an element has multiple class names, all will be replaced. That's why there are other properties.
- <b style="color: Plum;">elem.className</b>: A special object with methods to add/remove/toggle a single class. Besides, it is iterable, so we can list all classes with `for..of`.
- <b style="color: Plum;">elem.classList.add/remove(<span style="color: royalblue; font-weight: normal;">"class"</span>)</b>: Adds/removes the class.
- <b style="color: Plum;">elem.classList.toggle(<span style="color: royalblue; font-weight: normal;">"class"</span>)</b>: Adds the class if it doesn’t exist, otherwise removes it.
- <b style="color: Plum;">elem.classList.contains(<span style="color: royalblue; font-weight: normal;">"class"</span>)</b>: Checks for the given class, returns `true/false`.
...
- <b style="color: PaleGreen;">elem.cssText</b>: Used to set the full style as a string. Can append string as well like `elem.cssText += 'background: red'`. Just like `style` property, we cannot use it to read any css.
...
- <b style="color: Magenta;">scrollWidth & scrollHeight</b>: These properties represent the total width and height of the element's content, including overflow content that is not visible.
- <b style="color: Magenta;">scrollLeft & scrollTop</b>: These properties represent the number of pixels that the element's content is scrolled from its upper-left corner in the horizontal and vertical directions, respectively.
