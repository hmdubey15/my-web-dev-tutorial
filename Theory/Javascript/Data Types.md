...
- <b style="color: Salmon;">`str`.codePointAt(`pos`)</b>: Returns a decimal number representing the code for the character at position `pos`.
- <b style="color: Salmon;">String.fromCodePoint(`code`)</b>: Creates a character by its numeric `code`.
- <b style="color: Salmon;">`str`.substr(`pos`, `len`)</b>: Gives substring in `str` from `pos` of length `len`.
...
- <b style="color: Salmon;">`str`.includes(`str2`, `pos`)</b>: Returns `true/false` as per presence of string in `str`.
...
- <b style="color: Salmon;">`str`.replace(`str2`, `str3`)</b>: Replaces just first occurence of `str2` with `str3`.
- <b style="color: Salmon;">`str`.replaceAll(`str2`, `str3`)</b>: Replaces all occurences of `str2` with `str3`.
...
- <b style="color: Salmon;">`arr`.length</b>: Returns length of the array. It is writable: `arr.length=0` empties the array.
- <b style="color: Salmon;">`arr`.push(`ele`)</b>: Adds an element at the end.
...
Array destructuring is a JavaScript feature that allows you to extract values from arrays and assign them to variables in a concise and readable way. It enables you to unpack values from arrays into separate variables, making it easier to work with arrays and their elements. 

<b style="color: DarkOrchid;">Just check the below code and comments:</b>
...
console.log(animals); // animal1 -> 'cat'
...
- <b style="color: Salmon;">new Map([`key1`, `val1`], [`key2`, `val2`], .....)</b> **/** <b style="color: Salmon;">new Map(`anotherMap`)</b>: creates the map.
...
- <b style="color: Salmon;">`map`.values()</b>: returns an iterable for entries `[key, value]`, it’s used by default in `for..of`.
...
for (let entry of map) { // the same as of map.entries()
