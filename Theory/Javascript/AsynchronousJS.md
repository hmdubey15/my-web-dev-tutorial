...
<h4>IMPLEMENTATION OF Promise.all</h4>
...
The keyword `async` before a function makes the function return a promise. Other values (non-promise ones) are wrapped in a resolved promise automatically. In below given code `myFunction1` and `myFunction2` are same.
...
<span style="color: Crimson;">If promise is not returned, JS automatically wraps it in a promise which is resolved with `undefined` value.</span>
