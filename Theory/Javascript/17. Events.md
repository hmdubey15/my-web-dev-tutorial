...
- `event.target`: The element on which the event originally occurred. This is useful for determining which element was directly interacted with.
- `event.currentTarget`:  The element whose event listener triggered the event handler. This might differ from target in cases of event bubbling or capturing.
- `event.clientX / event.clientY`: Window-relative coordinates of the cursor, for pointer events.
- `event.timeStamp`: The time at which the event was created (in milliseconds since the epoch).
...
3. <span style="color: Aquamarine;">Check Target Element:</span> Inside the event handler attached to the parent element, you can inspect the event object to determine which specific child element triggered the event (the event target). You can then perform the desired action based on the event target. <span style="color: Gold;">One common way of checking that specific child is using `dataset` as identifiers on those children.</span>
