...
<span style="color:Orange;">Then only difference between function declarations and function expressions is that function declarations are hoisted and we can use them before defining, whereas function expressions (just like any variables [`let`, `const`, `var`]) are also hoisted but since their initialization will not be hoisted, we can't use them before defining.</span>
